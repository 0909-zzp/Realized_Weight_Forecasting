...
### Table 4 — 消融分析 (最终, 2026-07-16)

消融链路: M2 → M3 (+外生变量) → M3a (+自环豁免) → M4 (+网络惩罚)

| Step | ΔMSE | 相对改善 | DM pairwise | 解释 |
|:---:|---:|---:|---:|---|
| M2→M3 | −3.54e-7 | +1.55% | −20.77 | +外生变量 |
| M3→M3a | −1.27e-6 | +5.64% | −22.10 | +自环豁免 (最大贡献, ~73%) |
| M3a→M4 | −1.17e-7 | +0.55% | −11.95 | +网络惩罚 |
| **M2→M4** | **−1.74e-6** | **+7.61%** | **−23.16** | **全部组件合计** |

> **核心结论**: 自环豁免贡献最大 (73%总改善)，网络惩罚虽最小但高度显著 (p≈10⁻³²)。三个组件均提供正向且统计显著的边际贡献，消融证据链完整。
