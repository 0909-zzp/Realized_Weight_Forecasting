...
| §5.4 | 消融分析 (Table 4) ✅ | 拆解 M4→M3a→M3→M2 各组件贡献 → `消融分析/Table4_消融分析.py` |
...
### Table 2 — OOS 预测精度 (最终, MCS正式)

| # | Model | MSE | DM | MCS p-val | MCS rank | 90%MCS |
|:---:|---:|---:|---:|---:|:---:|:---:|
| 1 | VAR (OLS) | 9.76e-5 | — | 0.0625 | 7 | ❌ |
...
> MCS: Hansen-Lunde-Nason (2011) block bootstrap, 2000次。M4 唯一 p=1.0 模型。LSTM 虽 MSE 高但因日间波动极大而无法被统计区分。
...
### Table 4 — 消融分析 (DFL基线减法, 2026-07-16)

从完整模型 (M5+DFL) 逐组件减法，关注组合经济表现。
...
> DFL贡献最大 (夏普改善0.329). 外生变量次之 (改善0.061). MSE_w上升验证精度-收益权衡.
