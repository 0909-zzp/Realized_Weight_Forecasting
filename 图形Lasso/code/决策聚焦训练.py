...
def main():
    ...
    coef_path = feat_dir / "varx_coefs.npy"

    if not coef_path.exists():
        log("错误: VARX 系数文件不存在，请先运行 VARX预测.py")
        return

    coefs      = np.load(feat_dir / "varx_coefs.npy")
    intercepts = np.load(feat_dir / "varx_intercepts.npy")
    X_all      = np.load(feat_dir / "X_features.npy")
    Y_all      = np.load(feat_dir / "Y_targets.npy")
    log(f"加载完成: X={X_all.shape}  Y={Y_all.shape}")

    # 用全部样本做一次全量预测
    log("全量预测...")
    Y_pred = np.zeros((X_all.shape[0], K))
    for i in range(K):
        Y_pred[:, i] = X_all @ coefs[i] + intercepts[i]
