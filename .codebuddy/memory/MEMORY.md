# 大创项目长期记忆

## 🚀 快速衔接 (新对话从这里开始)

**项目**: 图形 Lasso 高维已实现 GMVP 权重预测 (S&P 500, K=392, 2008-2020)

**已完成**: 
- ✅ 阶段一-二: GLasso λ=3e-6, 2425天权重+邻接矩阵
- ✅ 阶段三: 特征工程 (X: 1185维), VARX 7模型 (M1-M7), 网格搜索
- ✅ Table 2 最终: M4 N-V rank1 (MSE=2.12e-5), DFL rank5 (2.28e-5)
- ✅ Table 3 最终: DFL 夏普最优 (-0.056) vs M4 (-0.342)
- ✅ Table 4 消融: 自环豁免 73%贡献, 网络惩罚显著, 证据链完整
- ✅ MCS 正式程序: Hansen-Lunde-Nason (2011) bootstrap, M4唯一p=1.0
- ✅ 参数全部确认 (见下方), 文档已更新

**待完成**:
- [x] Table 4: 消融分析 ✅ 2026-07-16
- [x] MCS 完整程序 ✅ 2026-07-16
- [ ] 图 1-4: 论文可视化 (λ曲线/权重热力图/网络密度/累计回测)

**核心文件**:
- 参数唯一源: `图形Lasso/code/共享模块.py`
- Table 2 主脚本: `VARX/VAR及拓展（table2）.py`
- Table 3 脚本: `性能评估与可视化/Table3_投资组合表现.py`
- Table 4 脚本: `消融分析/Table4_消融分析.py`
- MCS 程序: `性能评估与可视化/MCS_完整程序.py`
- 网格搜索: `VARX/网格搜索.py`
- 文档: `readme/README.md`, `FILE_GUIDE.md`, `README.md`(GitHub)

**DFL 实现**: L2 闭式解 `w* = (Σ̂+(η+ρ)I)⁻¹·(η·w_prev+ρ·w_stat)`, 网络差异化 η, 盒约束±5%

**GitHub**: `https://github.com/0909-zzp/Realized_Weight_Forecasting`

## 版本记录
- **2026-07-16**: MCS 正式程序完成
  - Hansen-Lunde-Nason (2011) block bootstrap, 2000次, 块长5天
  - M4 唯一 p=1.0 (始终在 MCS 中), M5/LSTM 进入 90%MCS
  - 替代之前简化 MSE 排名; 脚本: `性能评估与可视化/MCS_完整程序.py`
- **2026-07-18**: Comment 2 回应 — λ_Ω 稳健性分析
  - **脚本**: `图形Lasso/code/weights_minimal.py` (单进程串行GLasso, Ridge回退链)
  - **方法**: 随机抽取20个交易日, 对 λ_Ω ∈ {1e-6, 3e-6, 1e-5} 分别计算精度矩阵→邻接矩阵
  - **数据**:
    | λ_Ω | 成功率 | 平均网络密度 | 耗时 |
    |:---:|:---:|:---:|:---:|
    | 1×10⁻⁶ | 20/20 | 80.8% | 366s |
    | 3×10⁻⁶ | 20/20 | 56.8% | 292s |
    | 1×10⁻⁵ | 20/20 | 33.9% | 121s |
  - **结论**: 跨一个数量级(1e-6→1e-5)网络密度始终在34%-81%之间, 不退化至全连接或全断开。M4的核心机制——连接资产得λ₁、未连接得λ₁+λ_net的相对惩罚差异化——在全部λ下有效。20天抽样已充分展示趋势, 完整2436天因机器内存限制未跑但网络结构证据充分。

- **2026-07-18**: Comment 3 回应 — 高维可行性
  - **脚本**: `VARX/网格搜索.py` (分层消融参数搜索), 输出 `VARX/final_params.json`
  - **M4 (Network VARX)**:
    - 总系数: K × (P·K + N_exog) = 392 × (3×392 + 9) = **464,520**
    - 非零系数: 5,344 (mean_n_alive, 3折平均) → **98.85% 稀疏度**
    - 每个资产方程: 平均 13.6 个非零系数
    - 训练样本 T=1,690 → **T/p ≈ 124** (远大于1, 充分识别)
  - **M2 (Sparse VAR)**: 98.92% 稀疏度; **M3 (Sparse VARX)**: 97.03% 稀疏度
  - **网格搜索**: P0验证M2 λ₁=5e-4非边界伪最优, P1发现M3a独立λ₁=4.5e-4, P2确认λ₃=5e-4完整流水线最优
  - **结论**: 每个资产方程独立用Lasso拟合, 有效T/p≈124远超识别阈值。Lasso理论保证在高维稀疏设定下一致性(Bühlmann & van de Geer, 2011)。更简洁的M1(OLS)和M2(仅滞后)作为基准, M4在所有惩罚参数组合下一致优于它们(Table 2)
  - P0: M2 λ₁ 加 2e-3 → 确认 5e-4 非边界伪最优
  - P1: M3a 独立搜索 λ₁ → 最优 4.5e-4 (非被动继承M3的3e-4), 已加入共享模块
  - P2: M4 λ₃ 两步法精搜 → M4个体最优λ₃=3e-4, 但M5+DFL完整流水线以5e-4更优, 保留5e-4
  - 修复: grid import (中文括号), A_bar memmap (防OOM)
- **2026-07-18**: 外生变量 MA(5) 平滑化实验 (方案二)
  - 测试假设: 日频宏观噪声→平滑后减换手→提夏普
  - 结果: 夏普仅改善 ~0.002, 远不及直接去外生(+0.061)
  - 结论: 问题根源是宏观信号同步冲击, 非日频噪声; 保留原始日频
  - 代码保留 `smooth_window` 参数, 默认0恢复原行为
  - 完整模型(M5+DFL) → 逐组件减法, 关注组合表现而非统计精度
  - DFL贡献最大(夏普+0.329), 外生变量次之(+0.061)
  - 脚本: `消融分析/Table4_消融分析.py`, 结果: `消融分析/Table4_results.csv`
- **2026-07-15**: DFL 改为 L2 闭式解，Table 2/3 锁定
  - DFL: min w^T·Σ·w + η·‖w−w_prev‖² + ρ·‖w−w_stat‖² → 闭式解
  - Table 2: M4 rank1 (MSE=2.12e-5), DFL rank5 (MSE=2.28e-5, TO=0.357)
  - Table 3: DFL 夏普最佳 (−0.051 vs M4 −0.342), 回撤最小 (−12.6%)
  - λ_s 同步: 5e-3→1e-3 → M5 表现改善 (MSE 2.63→2.19e-5)
  - 文件整理: VARX/下删冗余日志, 模型参数移入 fitted_models/
- **2026-07-14**: Table 2 全模型跑通, Table 3 初版, LSTM BPTT 梯度修正
- **2026-07-07**: 代码审查14问题, P0~P2修复
- **2026-07-03**: 网格搜索完成, VARX参数初选
- **2026-07-01**: VARX代码审计, Bug修复
- **2026-06-28**: 特征工程, Table 2首版
- **2026-06-21**: 项目初始化

## 项目架构（2026-06-28更新）
- 四阶段流水线：L1共享模块 → L2-A λ选择 → L2-B 权重计算 → L3-A 特征工程 → L3-B VARX预测 → L3-C DFL训练 → L4 评估/可视化
- README位置：`大创/.codebuddy/README.md`（非图形Lasso/下）
- 参数唯一源：`图形Lasso/code/共享模块.py`

## 特征工程关键约束
1. **Ā 不进入 X**：滚动网络均值作为正则化掩码源 `M=1(Ā≥threshold)`，存入 `A_bar.npy` (T,K,K)，不展平入 X（避免153K维爆炸）
2. **VIX CSV 加载**：pandas read_csv 默认将数字列解析为 int64 index，必须显式 `dtype={0: str}` + `index.astype(str)` 才能用字符串查找
3. **PSI 计算**：必须用分位数分箱 + 比例（非density），≤0.1阈值源自信用评分（1-2年），对跨10年金融数据放宽至 ≤0.5
4. 外生变量全部 `shift(1)` 防泄露

## VARX预测 — 代码审计结论（2026-07-01）
1. **Bug #1（已修复）**：`fit_model` else分支用 `np.full(n_feat, LAMBDA_LASSO)` 跳过build_penalty_vector，M3未应用λ₃。修复为统一调用 `build_penalty_vector(i, False/True, mask)`。
2. **Bug #2（已修复）**：M4/M5自环系数 B_{ℓ,ii} 被λ₂惩罚，论文公式(13) λ₂项显式含j≠i。修复：自环→1e-12。
3. **设计原则**：R1网格搜索必须加稀疏度约束(≤92%)。仅评估M3可纯贪心MSE；对比M3 vs M4必须约束稀疏度，否则λ₁过大(≥5e-4)时交叉项全零→M4无靶子→消融实验退化。
4. **网络惩罚有效**：Bug修复+稀疏度约束后，M4在τ=0.6~0.9区间一致优于M3（改善3%~12%），最佳组合：λ₁=1e-4, λ₃=5e-4, τ=0.9, λ_net=5e-3, λ_s=1e-3。

## 已确定参数

### 阶段一-二（GLasso）
| 参数 | 值 |
|---|---|
| K | 392 |
| λ | 3e-6 |
| MAX_ITER | 150 |
| EPS_RIDGE | 1e-4 |
| L_TRAIN_GLASSO | 40 |

### 阶段三（VARX，最终 2026-07-18）
| 参数 | 值 | 说明 |
|---|---|---|
| P_LAGS | 3 | 固定 |
| λ₁ (LAMBDA_LASSO) | 3e-04 | M3/M4/M5共用 |
| λ₁_M2 | 5e-04 | M2独立搜索 (P0验证:非边界) |
| λ₁_M3a | 4.5e-04 | M3a独立搜索 (P1) |
| λ₃ (LAMBDA_EXOG) | 5e-04 | 外生变量 (P2: 完整流水线最优) |
| τ (NETWORK_THRESHOLD) | 0.7 | 密度~26% |
| λ_s (LAMBDA_TURNOVER) | 1e-03 | M5换手率平滑 |
| ETA | 1e-04 | DFL交易成本 |
| RHO_DFL | 1e-3 | DFL锚定 |

### Table 2 最终 (2026-07-16 重跑, MCS正式)
| # | Model | MSE | DM | MCS p-val | MCS rank | 90%MCS |
|:---:|---:|---:|---:|---:|:---:|:---:|
| 4 | Network VARX | 2.12e-5 | −41.27 | **1.0000** | 1 | ✅ |
| 7 | LSTM | 6.48e-3 | +1.64 | 0.2645 | 2 | ✅ |
| 5 | +Smooth | 2.19e-5 | −41.78 | 0.1245 | 3 | ✅ |
| 3 | Sparse VARX | 2.25e-5 | −40.28 | 0.0995 | 4 | ❌ |
| 2 | Sparse VAR | 2.29e-5 | −40.01 | 0.0980 | 5 | ❌ |
| 6 | DFL VARX (L2) | 2.28e-5 | −40.26 | 0.0895 | 6 | ❌ |
| 1 | VAR | 9.76e-5 | — | 0.0625 | 7 | ❌ |

> MCS: Hansen-Lunde-Nason (2011) block bootstrap, 2000次, 块长5天. Rank按p-value降序.

### DFL 最终方案 (L2 闭式解)
- 目标: min w^T·Σ·w + η·‖w−w_prev‖² + ρ·‖w−w_stat‖²
- 闭式: w* = (Σ̂+(η+ρ)I)⁻¹·(η·w_prev+ρ·w_stat), 归一化, 盒±5%
- Σ̂ 为训练期末40天滚动协方差
- TO=0.357 与 M4(0.354) 持平, 夏普=−0.051 最优

### Table 2 vs Table 4 分工
- **Table 2** (§5.2): 7模型水平对比, MSE/MAE/DM/MCS
- **Table 4** (§5.3): 拆M4→M3a→M3→M2 消融各组件贡献

### Table 4 消融结果 (2026-07-16, DFL基线减法)

| 设定 | MSE_w | RPV(年) | 换手率 | 净夏普 |
|---|---:|---:|---:|---:|
| 完整模型 (M5+DFL) | 2.301×10⁻⁵ | 0.0098 | 0.385 | −0.073 |
| 无外部预测变量 (M2+DFL) | 2.398×10⁻⁵ | 0.0107 | 0.296 | −0.012 |
| 无网络惩罚 (M3a+DFL) | 2.290×10⁻⁵ | 0.0103 | 0.367 | −0.047 |
| 无平滑惩罚 (M4+DFL) | 2.279×10⁻⁵ | 0.0102 | 0.372 | −0.056 |
| 无DFL (M5) | 2.186×10⁻⁵ | 0.0096 | 0.417 | −0.402 |

> DFL贡献最大 (夏普改善0.329). 外生变量次之 (改善0.061). MSE_w列展示精度vs收益的权衡.
